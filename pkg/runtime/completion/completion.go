...
func createAURList(aurURL string, out *os.File) error {
	...
	scanner.Scan()
	for scanner.Scan() {
		_, err = out.WriteString(scanner.Text() + "\tAUR\n")
		if err != nil {
			return err
		}
	}
...
func createRepoList(alpmHandle *alpm.Handle, out *os.File) error {
	...
	_ = dbList.ForEach(func(db alpm.DB) error {
		_ = db.PkgCache().ForEach(func(pkg alpm.Package) error {
			_, err = out.WriteString(pkg.Name() + "\t" + pkg.DB().Name() + "\n")
			return err
		})
		return nil
	})
